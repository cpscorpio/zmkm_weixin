...
//                db.run("update token set exp_time=" + date + " where token='" + code + "'");    //设置为过期
                var updateTime = db.prepare("UPDATE token SET exp_time=? where token=?");
                updateTime.run(date -1, code);
                updateTime.finalize();
                ...
                utils.get(qrUrl( code, consts.TIME_OUT), function ( data)
                {
                    try{
                        ...
                        var db = new sqlite.Database(dateFile);
                        var stmt = db.prepare("update token set exp_time=?, token=?, qr_url=? where door=? ");
                        stmt.run( expTime, code, data.data.qr_url, row[0].door);
                        stmt.finalize(function()
                        {
                        });
                        cb(err, row[0]);
