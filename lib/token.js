...
var dateFile = __dirname + "/../data.sqlite";
...
function createTable(id, cb)
{
    ...
    db.serialize(function() {
        db.run("CREATE TABLE token (token TEXT,exp_time integer,door integer,id integer NOT NULL PRIMARY KEY AUTOINCREMENT)");
        var expTime = Math.ceil(new Date().getTime()/1000) + consts.TIME_OUT;
        var stmt = db.prepare("INSERT INTO token (exp_time, token,door) VALUES (?,?,?)");
        stmt.run( expTime, _getRandomString(),id);
        ...
        db.all("SELECT * FROM token where door=" + id + " and exp_time > " + date, function(err, row) {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var exp_time = Math.ceil(new Date().getTime()/1000) + consts.TIME_OUT;
                    var stmt = db.prepare("INSERT INTO token (exp_time, token, door) VALUES (?,?, ?)");
                    stmt.run( exp_time , code, id);
                    stmt.finalize(function()
                    {
                        cb({
                            exp_time:exp_time,
...
token.check = function ( code, cb)
{
    ...
    db.serialize(function() {
        ...
        db.all("SELECT * FROM token where exp_time > " + date + " and token='" + code + "'", function(err, row)
        {
            ...
            if(row && row.length > 0)
            {
                db.run("update token set exp_time=" + date + " where token='" + code + "'");
                var expTime = Math.ceil(new Date().getTime()/1000) + consts.TIME_OUT;
                var stmt = db.prepare("INSERT INTO token (exp_time, token,door) VALUES (?,?,?)");
                stmt.run( expTime, _getRandomString(),row[0].door);
                stmt.finalize(function()
                {
                });
                cb(err, row[0]);
